...
test_print(){
	if [[ $ec2_hostname != "" ]]; then 
		printf "%-23s %-15s %-21s %-18s" $ec2_hostname $ec2_pri_ip $ec2_ami_id $ec2_key_name #$ec2_user $key_pair
		echo ""
	fi
}
test_line(){
...
ssh_remote_run(){
	if [[ $ec2_hostname != "" ]]; then 
		...
		#ssh -oStrictHostKeyChecking=no -i /data_backup/key/$key_pair $ec2_user@$ec2_pri_ip "$remote_command" < /dev/null >> ssh_command_log.txt
		ssh -oStrictHostKeyChecking=no -i /data_backup/key/$key_pair $ec2_user@$ec2_pri_ip "$remote_command" < /dev/null >> ${backup_dir_log}${ec2_hostname} 
	#	scp -oStrictHostKeyChecking=no -i /data_backup/key/$key_pair $ec2_user@$ec2_pri_ip:/data/AAAAAAAAAAAAAAA ${backup_dir_region}${ec2_hostname}/${ec2_hostname}_${nowtime}.tar.gz
	fi
}
...
ssh_remote_run #test remote command chk
